...
export function validateUrlInput(raw: string): URL {
  ...
  if (!trimmed) {
    throw new AuditError("INVALID_URL", "URL is required", 400);
  }
  ...
  try {
    ...
    throw new AuditError("INVALID_URL", "URL is not well-formed", 400);
  }
  ...
  if (!parsed.hostname) {
    throw new AuditError("INVALID_URL", "URL must include a hostname", 400);
  }
